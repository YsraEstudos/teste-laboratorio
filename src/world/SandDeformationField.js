...
export class SandDeformationField {
    ...
    brush(x, z, radius, depth, berm = 0, compression = 0, yaw = 0, elongation = 1, edge = 0) {
        ...
        const minPy = clamp(Math.floor(((z - radiusZ - this.minZ) / worldHeight) * this.resolution), 0, this.resolution - 1);
        ...
        for (let py = minPy; py <= maxPy; py += 1) {
            ...
            for (let px = minPx; px <= maxPx; px += 1) {
                ...
                const nextBerm = Math.max(this.bermData[index], Math.round(clamp(berm / this.maxDepth, 0, 1) * (0.2 + falloff * 0.8) * 255));
                const nextCompression = Math.max(this.compressionData[index], Math.round(clamp(compression, 0, 1) * falloff * 255));
                if (nextDepth !== this.data[index] || nextBerm !== this.bermData[index] || nextCompression !== this.compressionData[index]) {
