...
export class SandDeformationSimulation {
  ...
  constructor({ renderer, scene = new THREE.Scene(), resolution = 256 } = {}) {
    ...
    this.writeIndex = 0;
    this.brushCount = 0;
    this.brushData = new Float32Array(SandDeformationSimulation.MAX_BRUSHES * 2 * 4);
  ...
  queueBrush(x, z, radius, depth, berm = 0, compression = 0, yaw = 0, elongation = 1, edge = 0) {
    if (this.brushCount >= SandDeformationSimulation.MAX_BRUSHES) return false;
    const offset = this.brushCount * 4;
    const materialOffset = (SandDeformationSimulation.MAX_BRUSHES + this.brushCount) * 4;
